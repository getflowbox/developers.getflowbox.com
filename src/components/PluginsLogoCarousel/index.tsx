...
export default class PluginsLogoCarousel extends React.Component {
  constructor (props) {
    super(props)
      this.state = {
          position: -0,
  ...
  this.list = () => (
    ...
    {this.props.logos.map((value, index) => (
      <li key={index}><a href={value.url} target="_blank" rel="noopener noreferrer"><img src={'/img/pluginlogos/' + value.img} alt={value.alt} /></a></li>
    ))}
    </ul>
  )
  ...
  return (
    <div className={styles.companyUsage} ref={this.containerRef}>
      <h3>eCommerce Plugins and supported platforms</h3>
